...
if (_isHandheld) then {
    ...
    private _sideID = [WEST,EAST,INDEPENDENT,CIVILIAN] find (side _unit);
    if (_sideID < 0) exitWith {ERROR_1("Unit %1 unknown side.",_unit)};
    private _radioArrayVarName = [QGVAR(radiosWest),QGVAR(radiosEast),QGVAR(radiosIndependent),QGVAR(radiosCivilian)] select _sideID;
